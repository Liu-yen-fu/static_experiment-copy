...
def main() -> int:
            ...
            for layout in (context["layouts"][layout_name],):
                for workload in selected[workload_type]["measurement"]:
                    for repetition in range(1, nested(config, "workloads.measurement.repetitions") + 1):
                        ...
                        if status == "completed": raw.update({"selected_interior": result_data.get("selected_interior_count", ""), "selected_leaf": result_data.get("selected_leaf_count", ""), "syscall_count": result_data.get("syscall_attempted_count", ""), "prefetch_elapsed_us": result_data.get("prefetch_elapsed_us", ""), "first_query_latency_us": metrics["first_query_latency_us"], "average_latency_us": metrics["average_latency_us"], "ops": metrics["ops"], "major_page_faults": metrics["major_page_faults"], "minor_page_faults": metrics["minor_page_faults"], "resident_after_cold_pages": metrics["resident_after_cold_pages"], "requested_selected_resident_ratio": requested if requested is not None else "", "successful_selected_resident_ratio": successful if successful is not None else ""})
    ...
    print("[postprocess 1/4] summarizing results", file=sys.stderr, flush=True)
    ...
    print("[postprocess 2/4] finding significant effects", file=sys.stderr, flush=True)
    subprocess.run([sys.executable, str(Path(__file__).with_name("find_significant_effects.py")), "--experiment-dir", str(experiment_root)], check=True)
    print("[postprocess 3/4] plotting trade-off", file=sys.stderr, flush=True)
    subprocess.run([sys.executable, str(Path(__file__).with_name("plot_tradeoff.py")), "--experiment-dir", str(experiment_root)], check=True)
    print("[postprocess 4/4] generating report", file=sys.stderr, flush=True)

...
正文僅概述其設計方法；完整演算法與實作細節可於 GitHub 中檢查。  
...
抽樣 seed 固定為 `20250620`，因此重複實驗會使用相同的 training 與 measurement workload selection。  
...
實驗中，如果要執行 prefetch ，方法有兩種，統稱為 backend 。  
`madvise` backend 對欲 prefetch 的 page 送出 `MADV_WILLNEED` advice；`pread` backend 則用 buffered `pread()` 同步讀取 pages。  
實驗同時記錄 prefetch elapsed time，並在結果中把 prefetch cost 納入。  
...
`offset_topk_interior` 與 `residency_topk` 的參數沒有窮舉所有可能的 `n`、`interior_k` 與 `leaf_k`。  
這是因為一來目前實驗已經包含 33,840 個 measurement cells ，如果要窮舉所有可能的參數實驗會來不及完成。  
二來實驗主要想比較 layout 與 prefetch strategy 的主要效果。  
...
cache 清除後不額外使用其他 cold advice，而是依賴工作站提供的 drop-cache helper 建立 cold-cache condition。  

drop-cache helper 對應 Linux 的 `sync && echo 3 > /proc/sys/vm/drop_caches`。  
...
本報告主要解讀兩個 latency 指標：`effective_first_query_latency_us` 與 `effective_average_query_latency_us`。  
前者關注 cold-cache 後第一筆 query 的等待時間，對應使用者重新切回 App 時最容易感受到的初始延遲。  
後者關注 1000 個 operations 攤開後的平均成本，用來觀察 prefetch 對整批 workload 的整體影響。  
由於冷啟動經過多久或多少查詢後才算脫離
這兩個指標都把 prefetch elapsed time 納入，避免只看到 query 變快，卻忽略 prefetch 本身也需要時間。  
...
其中 `candidate_latency` 是某個 candidate configuration 在目前 metric 下的 latency；`baseline_latency` 則是相同 workload file、repetition、workload type 與 memory condition 下，`original` layout 且不做 prefetch 的 baseline latency。  
因此，若目前 metric 是 `effective_first_query_latency_us`，兩者都使用 effective first-query latency；若目前 metric 是 `effective_average_query_latency_us`，兩者都使用 effective average-query latency。  

## 統計顯著性

實驗使用 paired design。  
每個 candidate cell 都與相同 measurement workload file、相同 repetition、相同 workload type 與 memory condition 下的 baseline 配對。  
這樣可以減少不同 workload file 因查詢 key 序列不同、page locality 不同而造成的 latency 差異。  
...
## 最佳 layout / strategy / backend 組合（含顯著性）
...
## layout 與 prefetch 補充分析

前面的最佳組合表回答的是「哪一整套 configuration 最快」。  
但是一整套 configuration 同時包含 database layout、prefetch backend 與 prefetch strategy。   
因此，看到某個組合變快時，還需要進一步檢查：改善比較像是來自資料排列本身，還是來自 prefetch。  
為了做這個檢查，本實驗使用同一份 `all_raw.csv` 做兩個額外比較。  

第一個比較只看資料排列，不做 prefetch。  
也就是比較 `vacuum` 或 `rewrite` 的 no-prefetch baseline，是否比 `original` 的 no-prefetch baseline 更快。  

第二個比較固定同一種 layout，再比較 prefetch 是否比該 layout 自己的 no-prefetch baseline 更快。  
這兩個比較使用和主要結果相同的 effective metrics 與顯著性判定方式。  

結果顯示，資料排列本身主要改善 average-query latency。  
在只看 layout 的比較中，共有 22 個 configurations 顯示 layout 改變後 latency 顯著下降；其中 `vacuum` 有 14 個，`rewrite` 有 8 個，而且這些改善全部出現在 `effective_average_query_latency_us`。  
也就是說，單純改變資料在 database file 中的排列方式，對整體平均查詢延遲有幫助，但沒有穩定改善第一筆查詢延遲。

相對地，prefetch 對 first-query latency 的作用更明顯。  
在固定 layout 後比較 prefetch 的分析中，顯著且改善的結果共有 996 個。  
其中最常出現的是 `madvise` 搭配 `residency_topk`：在 effective first-query latency 有 299 個顯著改善，在 effective average-query latency 有 254 個顯著改善。 
`offset_topk_interior` 也有穩定改善，但數量較少；`range_interior` 更少，表示把所有 interior pages 都抓進來不一定比選擇性 prefetch 更好。  
`pread` 也有改善案例，但數量明顯少於 `madvise`。  
因此，補充分析支持主要結果的解讀：`vacuum` 經常成為 average-query latency 的最佳 layout，不只是 prefetch 造成的假象；而 first-query latency 的大幅改善，主要仍來自 prefetch。  

整體來說，本實驗看到的是資料排列與預取共同形成的改善，而不是單一因素獨自造成全部效果。
...
不過，本報告沒有進一步把 page faults attribution 拆到 interior、leaf 或 overflow pages，因此僅是驗證合理性，而不是完整因果證明。  
...
https://github.com/Liu-yen-fu/static_experiment-copy.git

...
Orchestrator必須將人類可讀進度寫至stderr，不得混入cell stdout artifacts：

- Training階段顯示目前profile序號／總profile數、layout、workload type、memory condition，以及每份training snapshot為running或reused。
- Measurement階段開始前計算完整cell總數。
- Measurement進度顯示done／total、百分比、completed、skipped、failed與timeout累計，以及最近或正在處理的cell分類標籤。
...
```csv
cell_id,status,measurement_file,repetition,selected_interior,selected_leaf,syscall_count,prefetch_elapsed_us,first_query_latency_us,average_latency_us,ops,major_page_faults,minor_page_faults,resident_after_cold_pages,requested_selected_resident_ratio,successful_selected_resident_ratio
```
...
Raw CSV不新增合成欄位；`ops`是benchmark harness輸出的原始operation數。`summarize_results.py`必須由每個completed cell的既有raw欄位派生effective first-query與effective average-query指標，寫入summary與comparison CSV，並由report呈現。
...
```csv
experiment_id,cell_id,status,workload_type,layout,memory_condition,memory_limit_enabled,memory_max_bytes,strategy_key,backend,n,interior_k,leaf_k,measurement_file,repetition,selected_interior,selected_leaf,syscall_count,prefetch_elapsed_us,first_query_latency_us,average_latency_us,effective_first_query_latency_us,effective_average_query_latency_us,ops,major_page_faults,minor_page_faults,resident_after_cold_pages,requested_selected_resident_ratio,successful_selected_resident_ratio
```
...
- Effective average-query latency（將prefetch cost攤入每個operation）。
...
effective_average_query_improvement_percent =
    (baseline_average_latency_us - effective_average_query_latency_us)
    / baseline_average_latency_us × 100
```

Baseline沒有prefetch cost，其`effective_first_query_latency_us`等於`first_query_latency_us`，`effective_average_query_latency_us`等於`average_latency_us`。Effective improvement必須和相同layout、workload type、memory condition、measurement file與repetition的baseline latency配對後逐sample計算，再聚合其distribution。若prefetch、latency或`ops`缺失，對應cell的effective指標亦缺失。
...
Markdown report必須在詳細結果前，針對每個`workload type × memory condition`組合列出最佳layout、strategy與backend。至少包含兩個視角：

```text
最低 effective_average_query_latency_us median
最低 effective_first_query_latency_us median
```
...
Effective average-query latency median
Effective average-query improvement vs same-layout baseline
...
Effective average-query latency median
Effective average-query improvement vs shared baseline
Requested selected resident ratio
Successful selected resident ratio
```
...
Effective average-query latency median
Effective average-query change vs reference condition
```
